function [r,tau,xcorr_tmp_out] = gccphat3( x, y, fs,d,c,tmppha,xcorr_tmp)

M = max(numel(x),numel(y));
a = sum(abs(x(:)));
b = sum(abs(y(:)));
if sum(abs(x(:))) < 100/32767/M && sum(abs(y(:)))< 100/32767/M
    tau = tmppha;
    r=0;
else
...
% tmp2 = angle(tmp);
% tmp3 = tmp./(abs(tmp));
tmp_buffer = 0.8*xcorr_tmp + 0.2*tmp;
xcorr_tmp_out = tmp_buffer;
r2 = ifft(exp(1i*angle(tmp_buffer)));
% r2 = ifft(tmp);
rtmp = fftshift(r2);
range = Nfft/2+1-(N-1)/2:Nfft/2+1+(N-1)/2;
r = rtmp(range);
...
if k < 0.2 || tau0 < -threshold || tau0 > threshold
    tau = tmppha;
else
% tau = N/(2*fs)+lags(idx);
